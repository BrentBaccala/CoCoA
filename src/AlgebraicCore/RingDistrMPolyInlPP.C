...
namespace CoCoA
{
  ...
  /*-----------------------------------------------------------------*/
  /** \include RingDistrMPolyInlPP.txt  */
  /*-----------------------------------------------------------------*/
...
//    RingDistrMPolyInlPPImpl(const ring& R, const std::vector<symbol>& IndetNames, const PPOrdering& ord);
  ...
  void RingDistrMPolyInlPPImpl::myMoveLM(RawPtr rawf, RawPtr rawg) const
  {
...
void RingDistrMPolyInlPPImpl::myDivLM(RawPtr rawlhs, ConstRawPtr rawf, ConstRawPtr rawg) const
  ...
  void RingDistrMPolyInlPPImpl::myAddMul(RawPtr rawf, ConstRawPtr rawh, ConstRawPtr rawg) const //???? delete me???
  {
    import(rawf).myAddMul(import(rawh), import(rawg), /* SkipLMg = */ false);
  }


  void RingDistrMPolyInlPPImpl::myAddMul(RawPtr rawf, ConstRawPtr rawh, ConstRawPtr rawg, SkipLMFlag skip) const //???? delete me???
  {
...
void RingDistrMPolyInlPPImpl::myReductionStep(RawPtr rawf, ConstRawPtr rawg) const
  ...
  SparsePolyRing NewPolyRing_DMPI(const ring& CoeffRing, const std::vector<symbol>& IndetNames, const PPOrderingCtor& ord)
  {
    if (!IsCommutative(CoeffRing))
      CoCoA_ERROR(ERR::NotCommutative, "NewPolyRing_DMPI pseudo ctor");
    if (!AreGoodIndetNames(CoeffRing, IndetNames))
    ...
    return SparsePolyRing(new RingDistrMPolyInlPPImpl(CoeffRing, NewPPMonoidOv(IndetNames, ord.myCtor(len(IndetNames)))));
  }
  ...
  {
    //if (IndetNames.empty()) ????
    return NewPolyRing_DMPI(CoeffRing, IndetNames, NewStdDegRevLexOrdering(len(IndetNames)));
  }
  ...
  SparsePolyRing NewPolyRing_DMPI(const ring& CoeffRing, long NumIndets)
  {
    ...
    return NewPolyRing_DMPI(CoeffRing, SymbolRange("x", 0, NumIndets-1), NewStdDegRevLexOrdering(NumIndets));
  }
  ...
  SparsePolyRing NewPolyRing_DMPI(const ring& CoeffRing, long NumIndets, const PPOrderingCtor& ord)
  {
    ...
    return NewPolyRing_DMPI(CoeffRing, SymbolRange("x", 0, NumIndets-1), ord.myCtor(NumIndets));
  }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/RingDistrMPolyInlPP.C,v 1.40 2014/07/28 15:48:07 abbott Exp $
