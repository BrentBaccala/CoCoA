...
void TestPPMonoid(PPMonoid PPM)
{
  ...
  TEST_ASSERT(!IsIndetPosPower(x[0]*x[2]));
  TEST_ASSERT(!IsIndet(t1));
  TEST_ASSERT(!IsIndetPosPower(t1));
  {
    long n,d;
    TEST_ASSERT(!IsIndetPosPower(n,d,x[0]*x[2]));
    TEST_ASSERT(IsIndetPosPower(n,d,x[0]));
    TEST_ASSERT(IsIndetPosPower(n,d,x[NumIndets(PPM)-1]));
  }
  ...
  TEST_ASSERT(t1t2 == IndetPower(PPM, 0, exponent(t1t2,0))*
...
//                 {0, 1, 0, 0}};
                 {-1, 0, -2, -4}};
  PPOrdering MatOrd = NewMatrixOrdering(n,
                                        GradingDim, 
