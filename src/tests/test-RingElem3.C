...
void TestIsDivisible()
{
  ...
  TEST_ASSERT(!IsDivisible(a,b));
  TEST_ASSERT(!IsDivisible(a,a));
  TEST_ASSERT(!IsDivisible(b,b));
  TEST_ASSERT(!IsDivisible(0,a));
  TEST_ASSERT(!IsDivisible(0,b));
  TEST_ASSERT(!IsDivisible(0,zero(ZZmod6)));
}

void TestZeroPowerZero(ring R)
{
  const RingElem z = zero(R);
  TEST_ASSERT(IsOne(power(z,0)));
  TEST_ASSERT(IsOne(power(z,BigInt(0))));
}
