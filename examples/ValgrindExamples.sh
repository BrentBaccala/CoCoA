...
echo
echounderline "Running valgrind on the CoCoALib examples"
...
do
  ...
  if [ -f $prog.in ]
  then
    valgrind ./$prog < $prog.in  2>&1 | grep "definitely lost\|no leaks are possible"
  else
    valgrind ./$prog 2>&1 | grep "definitely lost\|no leaks are possible"
  fi
  /bin/rm -f $prog.cout
done
