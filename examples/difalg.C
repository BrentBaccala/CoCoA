...
/* A differential ideal is generated by addition, multiplication, and
 * differentiation.
 *
...
// differential ideals, which are in turn represented as an
...
class RegularDifferentialChain {
  friend std::ostream & operator<<(std::ostream &out, const RegularDifferentialChain);
public:
  //std::unordered_set<RingElem> equations;
  std::vector<RingElem> equations;

  RegularDifferentialChain(std::vector<RingElem> eq)
    : equations(eq.begin(), eq.end())
  {
  }
};

std::ostream & operator<<(std::ostream &out, const RegularDifferentialChain omega)
{
  out << "RegularDifferentialChain(";
...
public:
  ...
  virtual std::vector<RegularDifferentialChain>
  my_Rosenfeld_Groebner(std::vector<RingElem> equations, std::vector<RingElem> inequations) const;

  virtual RingElem myReduce(RingElem e, const RegularDifferentialChain& rs) const;
...
namespace diffalg {
  ...
  void Rosenfeld_Groebner(std::vector<RingElem> equations, std::vector<RingElem> inequations,
			  std::vector<RegularDifferentialChain> & results, int nesting_level=0)
  {
    ...
    if (R.empty() || IsZero(R[0])) {
      RegularDifferentialChain result(A, Union(partial_rem(inequations, A), h));
  ...
  {
    std::vector<RegularDifferentialChain> results;
...
#if 0
  std::vector<RegularDifferentialChain> Rosenfeld_Groebner(std::vector<RingElem> inequations)
  {
    return Rosenfeld_Groebner(gens, inequations);
  }

  std::vector<RegularDifferentialChain> Rosenfeld_Groebner(void)
  {
...
std::vector<RegularDifferentialChain> DifferentialRingBase::my_Rosenfeld_Groebner(std::vector<RingElem> equations, std::vector<RingElem> inequations) const
{
  ...
  for (int i=0; i < T->inter.size; i ++) {
    ...
    results.push_back(RegularDifferentialChain(v));
  }
...
// Compute reduced form of a RingElem against a RegularDifferentialChain
//
// Reduced form of p is a differential polynomial q, equivalent to p
// module the ideal defined by rs.

RingElem DifferentialRingBase::myReduce (RingElem e, const RegularDifferentialChain& rs) const
{
...
/* These are publicly callable versions of Rosenfeld_Groebner and
 * operator% that invoke the ring's virtual methods.
 */
...
std::vector<RegularDifferentialChain> Rosenfeld_Groebner(Args... rest) {
  std::vector<RingElem> gens{rest...};
  return Rosenfeld_Groebner(gens, std::vector<RingElem>());
}

RingElem operator% (RingElem e, const RegularDifferentialChain& rs)
{
  CoCoA_ASSERT(rs.equations.size() > 0);
  const ring& R = owner(rs.equations[0]);
...
void testDifferentialRing(void)
{
  ...
  std::vector<RegularDifferentialChain> Bo95_ex1_RG = Rosenfeld_Groebner(s1, s2);
