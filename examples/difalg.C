...
// systems.
//
// A regular system is a system of equations and inequations such that
// the equations are autoreduced and coherent, the initial and
// separant of each equation appears among the inequations, and any
// remaining inequations are partially reduced w.r.t. the equations.
// ([Bo95] Definition 2)
//
...
// equation's ideal colon-quotient'ed by the inequation's
// multiplicative family (i.e, the smallest multiplicatively closed
// set generated by the inequations).
//
// Note that this ISN'T the ideal quotient, since we can't use ideals
// with inequations (a <> 0 and b <> 0 doesn't imply a+b <> 0).
// However, colon quotients don't need ideals for their
// 'denominators'; only multiplicatively closed sets are required.
...
public:
  ...
  RegularDifferentialIdeal(const RingElem& r1, const RingElem& r2, const RingElem& r3, const RingElem& r4) : R(owner(r1)) {
    gens.push_back(r1);
    gens.push_back(r2);
    gens.push_back(r3);
    gens.push_back(r4);
